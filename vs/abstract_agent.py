...
class AbstAgent(ABC):
    ...
    def walk(self, dx, dy):
        ...
        @returns VS.BUMPED = the agent bumped into a wall or the end of grid
        @returns VS.TIME_EXCEEDED = the agent has no enough time to execute
                                    the action
    ...
    def check_walls_and_lim(self):
        ...
        @returns: a vector of eight integers indexed in a clockwise manner.
           ...
           position of the agent, the second is in the upper right diagonal
           direction, the third is to the right, and so on.
        Each vector position contains one of the following values:
    ...
    def read_vital_signals(self):
        ...
        @returns:
        - VS.TIME_EXCEEDED if the agent has no enough battery time to read the
          vital signals or
        - the list of vital signals (if there is a victim at the current
          agent's position) or
