...
class Env:
    def __init__(self, vict_folder, env_folder):
        ...
        self.obst = [
            [1 for y in range(self.dic["GRID_HEIGHT"])]
        ...
        if self.nb_of_victims > len(self.signals):
            print(
                "ENV: number of victims of env_victims.txt greater than vital signals"
            )
    ...
    def __draw(self):
        ...
        rect = pygame.Rect(
            self.dic["BASE"][0] * cell_w, self.dic["BASE"][1] * cell_h, cell_w, cell_h
        )
        ...
        for victim in self.victims:
            victim_rect = pygame.Rect(
                victim[0] * cell_w + 1, victim[1] * cell_h + 1, cell_w - 1, cell_h - 1
            )
            c = self.tri[v]
    ...
    def __print_victims(self, victims, type_str, sub, ident=3):
        ...
        if len(victims) > 0:
            ...
            if self.tri.count(0) > 0:
                print(
                    f"{idents}Green  {type_str}      (V{sub}0) = {tri.count(0):3d} out of {self.tri.count(0)} ({100 * tri.count(0) / self.tri.count(0):.1f})%"
                )
            if self.tri.count(1) > 0:
                print(
                    f"{idents}Yellow {type_str}      (V{sub}1) = {tri.count(1):3d} out of {self.tri.count(1)} ({100 * tri.count(1) / self.tri.count(1):.1f})%"
                )
            if self.tri.count(2) > 0:
                print(
                    f"{idents}Red    {type_str}      (V{sub}2) = {tri.count(2):3d} out of {self.tri.count(2)} ({100 * tri.count(2) / self.tri.count(2):.1f})%"
                )
            if self.tri.count(3) > 0:
                print(
                    f"{idents}Black  {type_str}      (V{sub}3) = {tri.count(3):3d} out of {self.tri.count(3)} ({100 * tri.count(3) / self.tri.count(3):.1f})%"
                )
            print(f"{idents}--------------------------------------")
            print(
                f"{idents}Total of {type_str} victims     (V{sub})  = {len(tri):3d} ({100 * float(len(tri) / self.nb_of_victims):.2f}%)"
            )

            weighted = (
                3 * tri.count(0) + 3 * tri.count(1) + 3 * tri.count(2) + tri.count(3)
            ) / (
                3 * self.tri.count(0)
                + 3 * self.tri.count(1)
                + 3 * self.tri.count(2)
            ...
            print(
                f"{idents}Weighted {type_str} victims per sobr (V{sub}g) = {weighted:.2f}\n"
            )
            print(
                f"{idents}Sum of sobr of all {type_str} victims = {tot_sobr:.2f} of a total of {self.sum_sobr:.2f}"
            )
            print(
                f"{idents}  % of sobr of all {type_str} victims = {tot_sobr / self.sum_sobr:.2f}"
            )
            print(f"{idents}--------------------------------------")
            print(f"{idents}CSV of {type_str} victims")
            print(f"{idents}V{sub}0,V{sub}1,V{sub}2,V{sub}3,V{sub}g")
            print(
                f"{idents}{tri.count(0)},{tri.count(1)},{tri.count(2)},{tri.count(3)},{weighted}"
            )
        else:
    ...
    def print_acum_results(self):
        ...
        print(
            f"   {self.tri.count(0)},{self.tri.count(1)},{self.tri.count(2)} {self.tri.count(3)},{self.sum_sobr}"
        )
